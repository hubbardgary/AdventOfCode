...
#
# For example, suppose you have 8 hit points, 5 damage, and 5 armor, and that the boss has 12 hit points, 7 damage, and
# 2 armor:
#
...
rings = [
    ...
    ["Defence+1 ring", 20, 0, 1],
    ["Defence+2 ring", 40, 0, 2],
    ["Defence+3 ring", 80, 0, 3]
]

boss_attack_points = 8
boss_defence_points = 1
boss_health_points = 104
player_health_points = 100

cheapest_victory = 9999
cheapest_victory_kit = ""
most_expensive_defeat = 0
most_expensive_defeat_kit = ""

for i in range(0, len(weapons)):
    for j in range(0, len(armour)):
        for k in range(0, len(rings)):
            for l in range(0, len(rings)):
                kit = "{0}, {1}, {2}, {3}".format(weapons[i][0], armour[j][0], rings[k][0], rings[l][0])
                cost = weapons[i][1] + armour[j][1] + rings[k][1] + rings[l][1]

                # Not allowed two of the same rings
                if rings[k][1] > 0 and rings[k][0] == rings[l][0]:
                    continue

                while player_health_points > 0 and boss_health_points > 0:
                    # Do battle!
                    boss_health_points -= max(1, weapons[i][2] + rings[k][2] + rings[l][2] - boss_defence_points)
                    if boss_health_points > 0:
                        # Boss still alive, so now he attacks
                        player_health_points -= max(1, boss_attack_points - (armour[j][3] + rings[k][3] + rings[l][3]))

                if boss_health_points < 1 and player_health_points > 0 and cost < cheapest_victory:
                    # Boss is dead and it's the cheapest victory so far
                    cheapest_victory = cost
                    cheapest_victory_kit = kit
                elif boss_health_points > 0 and player_health_points < 1 and cost > most_expensive_defeat:
                    # Player is dead and it's the most expensive defeat so far
                    most_expensive_defeat = cost
                    most_expensive_defeat_kit = kit

                # Resurrect boss and player for next round
                boss_health_points = 104
                player_health_points = 100

print("Part 1: Cheapest victory is {0} using {1}".format(cheapest_victory, cheapest_victory_kit))
print("Part 2: Most expensive defeat is {0} using {1}".format(most_expensive_defeat, most_expensive_defeat_kit))
