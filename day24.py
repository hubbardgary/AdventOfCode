...
# is, the value you get when you multiply their weights together. Only consider quantum entanglement if the first group
# has the fewest possible number of packages in it and all groups weigh the same amount.
#
# For example, suppose you have ten packages with weights 1 through 5 and 7 through 11. For this situation, the unique
...
# the ideal configuration is therefore 99. Had there been two configurations with only two packages in the first group,
...
#
# Given the example packages above, this would be the new unique first groups, their quantum entanglements, and one way
# to divide the remaining packages:
#
...
def target_weight(packages, sacks):
    return sum(packages) // sacks


def smallest_groups_of_target_weight(packages, weight):
    for i in range(0, len(packages)):
        p = [x for x in combinations(packages, i) if sum(x) == weight]
        if len(p) > 0:
            # We've found the groups with fewest packages, so these must go in the passenger seat
            return p


def lowest_quantum_entanglement(sack):
    lowest = float("inf")
    package_config = ()
    for packages in sack:
        quantum_entanglement = 1
        for p in packages:
            quantum_entanglement *= p
        if quantum_entanglement < lowest:
            lowest = quantum_entanglement
            package_config = packages
    return lowest, package_config

data = [int(line) for line in open("day24_input").readlines()]

# Part 1
groups = smallest_groups_of_target_weight(data, target_weight(data, 3))
print(lowest_quantum_entanglement(groups))

# Part 2
groups = smallest_groups_of_target_weight(data, target_weight(data, 4))
print(lowest_quantum_entanglement(groups))
