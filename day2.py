...
def get_ribbon_length(l, w, h):
    ...
    box_dims = [l, w, h]
    ribbon = min(box_dims) * 2
    box_dims.remove(min(box_dims))
    ribbon += min(box_dims) * 2
...
ribbon_length = 0
for box in boxes:
    box_dimensions = [int(b) for b in box.split("x")]
    paper_size += get_paper_size(box_dimensions[0], box_dimensions[1], box_dimensions[2])
    ribbon_length += get_ribbon_length(box_dimensions[0], box_dimensions[1], box_dimensions[2])

print("Part 1 - {0} square feet of wrapping paper".format(paper_size))
print("Part 2 - {0} feet of ribbon".format(ribbon_length))
